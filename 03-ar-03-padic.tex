...
$g'(x_k) = q x_k^{q-1} - 1$ are approximations of $q - 1$, which are 
defined over $\mathbf{Z}_p$ already.  Thus, we can replace the Hensel lifting 
iteration by 
\begin{equation}
...
on the expression 
\begin{equation}
\Sigma^k x = \sum_{i=0}^{\floor{d/B}-1} \Bigl( \sum_{j=0}^{B-1} a_{Bi + j} (\Sigma^k X)^j  \Bigr) X^{Bi},
\end{equation}
