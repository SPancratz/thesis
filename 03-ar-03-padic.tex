...
$p = 17$ and the precisions $N = 2^i$ for $i = 0, \dotsc, 19$.  In order to 
avoid problems associated with different random number sequences on the test 
systems, we consider the explicit elements $3^{3 N}$, $3^{6 N}$, 
$17 \times 3^{3 N}$, and $1 + 17 \times 3^{3 N}$ reduced modulo $17^N$ 
as input to the various arithmetic functions.  The timings are collected 
in Figure~\ref{fig:timings-qp}.
...
\centering
\includegraphics[width=0.9\textwidth]{bin/qp-inv.eps}
\end{minipage}
\begin{minipage}[t]{0.5\linewidth}
\centering
\includegraphics[width=0.9\textwidth]{bin/qp-sqrt.eps}
\end{minipage}\\

\begin{minipage}[t]{0.5\linewidth}
\centering
\includegraphics[width=0.9\textwidth]{bin/qp-teichmuller.eps}
\end{minipage}
\begin{minipage}[t]{0.5\linewidth}
\centering
\includegraphics[width=0.9\textwidth]{bin/qp-exp.eps}
\end{minipage}\\

\begin{minipage}[b]{0.5\linewidth}
\centering
\includegraphics[width=0.9\textwidth]{bin/qp-log.eps}
\end{minipage}
\begin{minipage}[b]{0.5\linewidth}
%
\end{minipage}
\caption{From left to right, top to bottom, we compute 
$\bigl(3^{3N}\bigr)^{-1}$, $\sqrt{3^{6 N}}$, the Teichm\"uller 
lift of~$3$, $\exp\bigl(17 \times 3^{3N}\bigr)$, and 
