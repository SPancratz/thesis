...
$f(X)$ is a dense polynomial.  In either case, the overall complexity of 
multiplication in $\mathbf{Z}_q$ modulo $p^N$ is given by 
$\mathcal{O}\bigl((d \log d) M(N \log p) \bigr)$.
...
determinant algorithm of Kaltofen~\citep{Kaltofen1992}, which requires 
...
Stockmeyer~\citep{PatersonStockmeyer1973} applied to the case 
of the $p$-adic exponential evaluates the truncated series 
...
There are two noticeable advantages to this approach.  Firstly, it 
...
$\mathcal{O}(\sqrt{n} \log n)$.  However, this does not lead to an 
asymptotic improvement.  We present the details in 
Algorithm~\ref{alg:exp-rectangular}.
