...
the action of $\Frob_p$ on $\Hrig^n(U) \cong \HdR^n(\mathfrak{U})$ 
to $p$-adic precision~$N$.

We now describe an explicit formula for the $b = \dim \HdR^n(\mathfrak{U})$ 
...
Then $\Frob_p (\omega_1) = 0$ unless, for all $i = 0, \dotsc, n$, 
...
the action of $\Frob_p$ on $\Hrig^{n}(U)$ using our previous results.
...
action of $\Frob_p$ on $\Hrig^{n}(U)$ in time complexity 
\begin{equation*}
