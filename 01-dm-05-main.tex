...
We let $\Phi$ denote the matrix for the $\sigma$-semilinear action 
...
on $H_{dR}^{n}(\mathfrak{U}/\mathfrak{S})$ has entries in 
$\mathbf{Q}_p\langle t, r(t)^{-1}\rangle^{\dagger}$, 
and let $\Phi_t$ denote the matrix for the action of $p^{-1} F_p$ 
on $H_{dR}^{n}(\mathfrak{U}_t)$ with entries in $\mathbf{Q}_p$.
Let us set 
\begin{align*}
r & = \ord_p((n-1)!), \\
...
the sum of two or three of $\ord_p(\Phi_0)$, $\ord_p\bigl(C \bmod t^K\bigr)$, 
and $\ord_p\bigl(C^{-1} \bmod{t^{\ceil{K/p}}}\bigr)$.
In order to compute the power series expansion around the origin 
of the matrix~$\Phi$ modulo~$t^K$ to $p$-adic precision~$N_2$, it 
suffices to compute the matrices $C$, $C^{-1}$ and $\Phi_0$ to 
$p$-adic precision $N_3$, $N_3'$ and $N_4$, respectively, where 
\begin{align*}
N_3  & \geq N_2 + \bigl(2\delta+(n-1)\bigr) \ceil{\log_p \ceil{K/p}} + \delta, \\
N_3' & \geq N_2 + \bigl(2\delta+(n-1)\bigr) \ceil{\log_p K} + \delta, \\
N_4  & \geq N_2 + \bigl(2\delta+(n-1)\bigr) \bigl(\ceil{\log_p K} + \ceil{\log_p \ceil{K/p}}\bigr).
\end{align*}
\end{cor}
...
In the previous section we described how to compute the matrix~$\Phi$ as 
...
on $H_{dR}^{n}(\mathfrak{U}/\mathfrak{S})$ are \emph{overconvergent 
functions}.  [[TODO:  Add a reference for this statement.]]
...
the entries of the matrix $\Psi(t) = r(t)^m \Phi(t)$ truncated 
modulo~$t^{K}$ allow us to compute the matrix $\Phi_{t_1}$ for 
the action of $p^{-1} F_p$ on $H_{dR}^{n}(\mathfrak{U}_{t_1})$ as 
...
on~$H_{dR}^{n}(\mathfrak{U}_{t_1})$ in terms of rational functions, 
we conclude this section by describing the evaluation at $t = t_1$.  
Modulo~$p^{N_2}$, the matrix $\Phi_1$ representing $p^{-1} F_p$ on 
$H_{dR}^{n}(\mathfrak{U}_{t_1})$ is equal to 
\begin{equation}
\Phi_1 = 
    r(\hat{t}_1)^{-m} 
    \Bigl( r(t)^m \Phi(t) \bmod{t^K} \Bigr) \Big\vert_{t=\hat{t}_1} \pmod{p^{N_2}}.
\end{equation}
...
\subsection{Computing the action of $q^{-1} F_q$ on $H(\mathfrak{U}_{t_1})$}

Let $\Phi_{t_1}$ and $\Phi_{t_1}^{(q)}$ denote the matrices representing the 
actions of $p^{-1} F_p$ and $q^{-1} F_q$ on $H_{dR}^{n}(\mathfrak{U}_{t_1})$, 
respectively.  Then 
\begin{equation*}
\Phi_{t_1}^{(q)} = 
...
on $H_{dR}^n(\mathfrak{U}_{t_1})$, much better bounds are available.
