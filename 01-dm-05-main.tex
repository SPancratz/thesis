...
like to determine integers~$K_1$ and~$K_2$ such that, modulo~$p^{N_2}$, 
the entries of the matrix $\Psi(t) = r(t)^m \Phi(t)$ truncated 
modulo~$t^{K_2}$ allow us to compute the matrix $\Phi_{t_1}$ for 
...
$p^{-1} F_p$ on $H_{dR}^{n}(\mathfrak{U}_{t_1})$ is equal to 
\begin{equation}
\Phi_{t_1} = 
    r(\hat{t}_1)^{-K_1} 
