...
\end{equation*}
using only $p$-adic inversion.  We observe that the summands are 
integers of size $\mathcal{O}(m \log m)$ bits, which allows us to 
avoid performing intermediate reductions modulo $p^{\tilde{N}}$.
\end{rem}
...
entries of~$F$ modulo~$p^N$ in Algorithm~\ref{alg:01-04-diagfrob}.

\begin{algorithm}
\caption{Compute the matrix for $p^{-1} F_p$ on $H_{dR}^n(\mathfrak{U})$}
\label{alg:01-04-diagfrob}
\begin{algorithmic}
\vspace{1mm}
...
[[TODO:  Include third example if its done by tonight.]]
